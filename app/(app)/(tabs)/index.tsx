...
const PermissionsScreen = () => {
  ...
  const StatCard = ({
    ...
    <View
      className={`p-4 rounded-xl bg-card w-[30%] shadow-sm shadow-border border border-border`}
    >
      <View className="items-center">
        ...
        <Text className="text-foreground text-2xl font-bold mt-1">{count}</Text>
      </View>
    </View>
  );
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    return (
      <View className="bg-card p-5 rounded-2xl mt-3 border border-border shadow-sm shadow-border/50">
        ...
        <View className="flex-row justify-between items-center pt-3 border-t border-border">
            <View className="flex-row items-center">
                ...
                <Text className="text-muted-foreground ml-2 text-sm">{fechaFormateada}</Text>
            </View>
            {item.aproved === null && (
                <TouchableOpacity onPress={() => handleCancelRequest(item.id)}>
                    <Text className="text-destructive text-sm font-semibold">Cancelar</Text>
                </TouchableOpacity>
            )}
        </View>
      </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="p-6 flex-1">
        ...
        <View className="flex-row justify-between items-start mb-8">
          <View>
            ...
            <Text className="text-base text-muted-foreground">
              Bienvenido, <Text className="text-primary font-medium">{username}</Text>
            </Text>
          </View>
          <TouchableOpacity
            onPress={handleViewChief}
            className="bg-card p-2 rounded-full border border-border shadow-sm"
          >
        ...
        <Pressable
          className="bg-primary p-4 rounded-xl flex-row justify-center items-center mb-8 shadow-lg shadow-primary/30 active:opacity-90"
             ...
             {chief ? (
                 <>
                    <View className="w-20 h-20 bg-primary/20 rounded-full items-center justify-center mb-4">
                        <Text className="text-3xl text-primary font-bold">{chief.name?.charAt(0)}</Text>
                    </View>
                    <Text className="text-xl text-foreground font-bold mb-1">{chief.name}</Text>
                    <Text className="text-muted-foreground mb-4">{chief.email}</Text>
                    <View className="flex-row items-center bg-secondary px-4 py-2 rounded-lg">
                        <Ionicons name="briefcase-outline" size={16} color="#64748b" />
                        <Text className="text-muted-foreground ml-2">{user?.section}</Text>
                    </View>
                 </>
             ) : (
                 <Text className="text-muted-foreground">No se encontró información del jefe de sección.</Text>
             )}
        </View>
      </CustomModal>
    </SafeAreaView>
